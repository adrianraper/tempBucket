	...
	public class DragProxy extends UIComponent {
		...
		private function _dispatchDragEvent(target:DisplayObject, event:DragEvent):void {
			...
			if (isSameOrChildApplicationDomain(target))
				...
				var mde:InterDragManagerEvent = new InterDragManagerEvent(InterDragManagerEvent.DISPATCH_DRAG_EVENT, false, false, event.localX, event.localY, event.relatedObject, event.ctrlKey, event.altKey, event.shiftKey, event.buttonDown, event.delta, target, event.type, event.dragInitiator, event.dragSource, event.action, event.draggedItem);
		...
		public function mouseMoveHandler(event:MouseEvent):void {
			...
			// #376
			if (dragSource.hasFormat("hitTestObjects")) {
				DragProxy.checkObjectsUnderPoint(dragSource.dataForFormat("hitTestObjects") as Array, stagePoint, targetList);
			} else if (dragSource.hasFormat("hitTestRoot")) {
				DragProxy.getObjectsUnderPoint(DisplayObject(dragSource.dataForFormat("hitTestRoot")), stagePoint, targetList);
			} else {
		...
		/**
		 * #376 - this does the same as getObjectsUnderPoint, but instead of checking everything from obj down, it only checks the explicit elements in objs
		 * 
		 * @param obj
		 * @param hitTestObjects
		 * @param pt
		 * @param arr
		 */
		private static function checkObjectsUnderPoint(objs:Array, pt:Point, arr:Array):void {
