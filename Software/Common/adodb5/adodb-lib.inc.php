...
function _adodb_column_sql(&$zthis, $action, $type, $fname, $fnameq, $arrFields, $magicq, $recurse=true) 
{
	...
	switch($type) {
		...
		case "T":
			// Clarity RM. Force this for SQLServer just in case there is a db that doesn't let us insert or update with a string.
			// Maybe it should be done as part of DBTimeStamp. Also, DK might prefer to comment it out since it doesn't impact him.
			if ($zthis->dataProvider=='mssql') {
				$val = "CONVERT(datetime, ".$zthis->DBTimeStamp($arrFields[$fname]).",120)";
			} else {
