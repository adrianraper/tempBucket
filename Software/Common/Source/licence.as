...
licenceNS.getConfirmLicence = function() {
	...
	loadVarsText.onData = function(raw) {
		...
		// v6.5.5.5 Need to convert this to a number for new code
		switch (this.master.licencing.toLowerCase()) {
			case "concurrent":
				this.master.licenceType=2;
				break;
			case "single":
				this.master.licenceType=4;
				break;
			default:
				this.master.licenceType=1;
		}
		...
		if (this.master.productCode==undefined){
			...
			case "holistic english":
				...
				break;
			case "sun on japanese":
				this.master.productCode = 41;
				break;
			// Sky - Don Friend
			case "english for hotel staff":
				...
				break;
			//Language Key
			case "language key hotel test":
				this.master.productCode = 42;
				break;
			// Protea
			case "issues in english two":
				this.master.productCode = 45;
				break;
			case "connected speech":
				this.master.productCode = 46;
				break;
	...
	if (_global.ORCHID.commandLine.licence==undefined) myTrace('licence=undefined');
	if (_global.ORCHID.commandLine.licence=="") myTrace('licence=""');
	if (_global.ORCHID.commandLine.licence==null) myTrace('licence=null');
	// v6.5.6 HCT problem. One college (MZC) can't get past this line, it tries to read licence. So add test for "" as well as undefined.
	// In fact, the above trace lines show that it is undefined AND null. In which case, why didn't it work?
		//_global.ORCHID.commandLine.licence!=undefined) {
	if ((_global.ORCHID.commandLine.prefix==undefined && _global.ORCHID.commandLine.rootID==undefined) || 
		(_global.ORCHID.commandLine.licence!=undefined && _global.ORCHID.commandLine.licence!="")) {
...
licenceNS.setProductBranding = function(productCode) {
	...
	case 34:
		...
		break;
	case 36:
		// v6.5.6 Change to match the interface
		//branding = "BC/ILA";
		branding = "BC/ILATest";
		break;
	case 35:
