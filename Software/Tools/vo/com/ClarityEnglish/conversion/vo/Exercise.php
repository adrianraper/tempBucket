...
class Exercise {
	...
	const EXERCISE_SECTION_HINT = 'hint';
	...
	const EXERCISE_TYPE_PROOFREADING = 'proofreading';
		...
		if ($xmlObj) {
			...
			switch (strtolower($attr['type'])) {
				...
				case 'analyze':
					...
					break;
				case 'stopdrop':
					$this->type = Exercise::EXERCISE_TYPE_DROPDOWN; //no this type in my canada
					$this->questionBased = true;
					break;
			  	...
			  	switch (strtolower($child->getName())) {
			  		...
			  		case Exercise::EXERCISE_SECTION_FEEDBACK:
						...
						//echo $child->getName() . " = " . $child->asXML() . "<br />";
			  			break;
					case Exercise::EXERCISE_SECTION_HINT:
			  			//$this->feedback = new Feedback($child, $this);
			  			if (!$this->hints) {
			  				$this->hints = new Hints($this);
			  			}
			  			$this->hints->addHint(new Hint($child, $this));
						//echo $child->getName() . " = " . $child->asXML() . "<br />";
			  			break;
			...
			$scoreBased = $this->settings->getSettingValue('feedback', 'scoreBased'); //I don't know what is scoreBased actually, but the output of layout seems okay
			/*if ($scoreBased)
