...
class Content{
		...
		if ($xmlObj) {
			...
			foreach ($xmlObj->children() as $child) {
				...
				if (strtolower($child->getName())=='paragraph') {
				    // Is it worth it? If I can drop paragraphs that are just for numbering that will get most of it?
                    $groupPara = New Paragraph($child, $this);
                    $this->addParagraph($groupPara);
                    /*
					// Combine paragraphs that are really one but Orchid had to separate
					// If there is an empty paragraph, that clearly separates two real ones.
					// If a paragraph has y="=", then it is really part of the previous one.
	...
	function output() {
		...
		if ((	($this->getParent()->getExerciseType()==Exercise::EXERCISE_TYPE_DRAGANDDROP) || 
				...
				($this->getParent()->getExerciseType()=='presentation')) &&
	...
	function bodyOutput($exerciseType) {
		...
		if ($this->parent->getType() == 'targetspotting' || $this->parent->getType() == 'proofreading')
			$buildText .= '<div class="section-content-container"><div class="section-content-body target-incorrect-area">';
		else
		...
		//	$exerciseType==Exercise::EXERCISE_TYPE_DROPDOWN) {
			...
			else //exercise type: presentation
			{
				foreach ($this->getParagraphs() as $paragraph) {
					if ($paragraph) {
						// Keep track of any paragraph that is a different tag type than the previous one
		...
		if (($exerciseType==Exercise::EXERCISE_TYPE_MULTIPLECHOICE) ||
            ...
            */
            ...
            foreach ($this->getParagraphs() as $paragraph) {
                ...
                // Get rid of <tab>, <b>#q</b>, <span>a.</span>
                $patterns = Array();
                $patterns[] = '/\<tab\>/is';
                $patterns[] = '/#q/is';
                $patterns[] = '/<span>[abcde]{1}\.<\/span>/i';
                $replacement = '';
				...
				$patterns[] = '/<p><\/p>/i';
                $replacement = '';
                $buildText = preg_replace($patterns, $replacement, $buildText);
