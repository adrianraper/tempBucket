...
%>
<%
Sub checkLicence(Query)
	Dim sql, userID, groupType, rootID
	sql = "SELECT F_LicenceID FROM T_User WHERE F_UserID=" & Clng(Query.UserID) & ";"
	rsProgress.open sql, connProgress, , adLockOptimistic
	
	If Not rsProgress.eof Then
		xmlNode = xmlNode & "<licence id='" & rsProgress.fields("F_LicenceID") & "' />"
	Else
		' AR not sure what the actual code should be
		xmlNode = xmlNode & "<err code='200' />"
	End If
	rsProgress.close
End Sub
...
Sub checkLogin(Query)
	...
	Else
		...
		Else
			...
			' AR Add double login check by adding F_LicenceID to the T_User table
			sql = "UPDATE T_User" &_ 
			" SET F_LicenceID=" & CLng(Query.LicenceID) &_
			" WHERE F_UserID=" & userID & ";" 
			Set ADOCmd= CreateObject("ADODB.Command")
			ADOCmd.ActiveConnection= Connection
			ADOCmd.CommandType= adCmdText
			ADOCmd.CommandText=sql
			
			Set rsResult= ADOCmd.Execute
			Set ADOCmd = nothing
...
Class XMLQuery
	...
	Private Sub fillInAttributes(node)
		...
		Select Case Ucase(node.nodeName)
			...
			Case "LICENCEID"
				LicenceID = node.firstChild.nodeValue
			Case "EKEY"
