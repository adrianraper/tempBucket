package org.davekeen.transitions {
	...
	/**
	 *  DK: The original SlideViewTransition class has a bug which causes a null pointer exception when transitioning a TabbedViewNavigator with its tab bar hidden.
	 *  This class is a direct copy of the original with that bug fixed.
	 * 
	 ...
	 */
	public class PatchedSlideViewTransition extends ViewTransitionBase {
		//--------------------------------------------------------------------------
		//
		//  Constructor
		//
		 ...
		 *  @productversion Flex 4.5
		 */
		public function PatchedSlideViewTransition() {
			super();
		...
		override public function captureStartValues():void {
			...
			if (tabBar && startView) {
				...
				animateTabBar = startView.overlayControls != endView.overlayControls || startView.tabBarVisible != endView.tabBarVisible;
			}
			...
			if (startView) {
				startViewProps = {includeInLayout: startView.includeInLayout, visible: startView.visible, cacheAsBitmap: startView.cacheAsBitmap, opaqueBackground: startView.opaqueBackground};
		...
		override protected function createViewEffect():IEffect {
			...
			if (startView) {
				...
				startViewProps.x = startView.x, startViewProps.y = startView.y;
			...
			if (endView) {
				endViewProps = {cacheAsBitmap: endView.cacheAsBitmap, opaqueBackground: endView.opaqueBackground};
		...
		override protected function createConsolidatedEffect():IEffect {
			...
			if (mode == SlideViewTransitionMode.COVER) {
				...
				navigatorProps.startViewCacheAsBitmap = (startView.contentGroup) ? startView.contentGroup.cacheAsBitmap : false;
				if (startView.contentGroup) startView.contentGroup.cacheAsBitmap = true;
			}
			
			if (endView) {
				navigatorProps.endViewCacheAsBitmap = (endView.contentGroup) ? endView.contentGroup.cacheAsBitmap : false;
				if (endView.contentGroup) endView.contentGroup.cacheAsBitmap = true;
			}
		...
		override protected function cleanUp():void {
			...
			if (!consolidatedTransition) {
				...
				if (startView) {
					if (startView.contentGroup) startView.contentGroup.cacheAsBitmap = navigatorProps.startViewCacheAsBitmap;
					startView.setLayoutBoundsPosition(navigatorProps.startViewX, navigatorProps.startViewY);
					startView.visible = true;
				}
				
				if (endView) {
					if (endView.contentGroup) endView.contentGroup.cacheAsBitmap = navigatorProps.endViewCacheAsBitmap;
				}
