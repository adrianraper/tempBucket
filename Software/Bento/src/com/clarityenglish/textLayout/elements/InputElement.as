	...
	public class InputElement extends TextComponentElement implements IComponentElement {
		...
		public override function set text(textValue:String):void {
			if (_hideChrome && value) {
				...
				// Pad the hidden text with a few _ characters (this is set using the after-pad CSS property).
                // If css: gap-text is set, then we just use that.
				// gh#1457 make sure that the longest correct answer will fit if you are using a fixed length gap
				// The text that you add here will never be seen, so it is just about getting the right sort of length.
				// Using the x character in the css is better than _.
				if (_gapText) {
                    if (_gapText.length < textValue.length)
                        _gapText = textValue + StringUtil.repeat("_", _gapAfterPadding);
                }
		...
		private function onEnter(event:FlexEvent):void {
			...
			if (event.target.focusManager) {
				if (scroller) {
					...
					//trace("children: "+event.target.parent.children);
					...
					if (nextComponent is Button) {
						...
						var stage:Stage = (nextComponent as ExerciseView).stage;
						stage.focus = null;
					}
