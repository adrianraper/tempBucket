package com.clarityenglish.textLayout.conversion {
	...
	public class XHTMLBlockImporter extends TextLayoutImporter {
		...
		public function createVideoFromXml(xmlToParse:XML):VideoElement {
			...
			if (xmlToParse.hasOwnProperty("@poster")) videoElement.poster = xmlToParse.@poster.toString();
			if (xmlToParse.item.length() > 0) videoElement.items = xmlToParse.item.copy();
		...
		public function createAudioFromXml(xmlToParse:XML):AudioElement {
			...
			if (xmlToParse.hasOwnProperty("@class")) audioElement.className = xmlToParse.@['class'].toString();
