...
this._fake._dInc=this._dInc;
    ...
    for (var i=0; i<ind; i++){
        for (var j=0; j<b_ar.length; j++){
            ...
            if (typeof b_ha[j][i] == "string") b_ha[j][i]=b_ha[j][i].replace(/\,/g,"\\,");
        }
        if (_isFF) b_ha[1][i]+=2;
   	...
   	this._fake._srnd=this._srnd;
				...
				for (i; i<rows.length; i++){
					...
					for (var j=0; j<max_ind; j++){
						...
						if (temp_rspan[j]) { 
							...
							if (_isIE || (_isFF && _FFrv >= 1.9 )) rows[i].insertBefore(document.createElement("TD"),rows[i].cells[0])
		...
		this.loadSizeFromCookie=function(name){
			...
			this.getChangedRows = function(){
				...
				this.forEachRow(function(id){
					...
					for (var j = 0; j < cols; j++) 
						if (j<ind && this._fake.rowsAr[id].childNodes[j].wasChanged){ res[res.length]=row.idd; break; }
						else if (row.childNodes[j].wasChanged){ res[res.length]=row.idd; break; }
				});
            ...
            this._fake.getRowById=function(id){
				var row = this.rowsAr[id]||this._fake.rowsAr[id];
      ...
      if (this._elmnh){
            ...
            this[zname]=function(){
				if (this.cellWidthType == "%"){
					...
					if (!_isIE&&wcor>1) wcor=wcor/2;
                    z.style.height=Math.max(Math.max(this.entBox.offsetHeight,0)-wcor,0)+"px";
                    if ((!this.noHeader) && this._fake.hdr.offsetHeight!=this.hdr.offsetHeight){
                    	if (this.hdr.rows.length!=2)
							for (var i=1; i<this._fake.hdr.rows.length; i++){
								this._fake.hdr.rows[i].style.height=this.hdr.rows[i].offsetHeight+"px";
								var check=this._fake.hdr.rows[i].offsetHeight-this.hdr.rows[i].offsetHeight;
								if (check && this.hdr.rows[i].offsetHeight > check) this._fake.hdr.rows[i].style.height=this.hdr.rows[i].offsetHeight-check+"px";
							}
						else
                    ...
                    if ((!this.noHeader) && this._fake.hdr.offsetHeight!=this.hdr.offsetHeight){
                    	if (this.hdr.rows.length!=2)
							for (var i=1; i<this.hdr.rows.length; i++){
								if (this.hdr.rows[i].offsetHeight<this._fake.hdr.rows[i].cells[0].scrollHeight)
									return this._fake.setSizes();
								this._fake.hdr.rows[i].style.height=this.hdr.rows[i].offsetHeight+"px";
								var check=this._fake.hdr.rows[i].offsetHeight-this.hdr.rows[i].offsetHeight;
								if (check) this._fake.hdr.rows[i].style.height=this.hdr.rows[i].offsetHeight-check+"px";
							}
						else
                        	this._fake.hdr.style.height=this.hdr.offsetHeight+"px";
                        this._fake["setSizes"].apply(this._fake,arguments);
            ...
            if (!ev.shiftKey){
                if (this._realfake){
                    if ((this.cell)&&(this.cell._cellIndex==(ind-1))){
                       ...
                       this._fake.selectCell(this.rowsCol._dhx_find(this.cell.parentNode),ind_t,false,false,true);
                       return false;
                       }
				  else
...
this._fake.sortField = function(a,b,c){ 
	...
	if (this.fldSort[a]!="na")
		this.setSortImgState(true,arguments[0],this._fake.getSortingState()[1])
					}
...
dhtmlXGridObject.prototype._correctSplit=function(a){
    ...
    pa.style.width=Math.max(0,this._fake.entBox.offsetWidth-a-(this._fake._sizeFix?this._fake._sizeFix*2:0))+"px";
    }
}
/*
...
dhtmlXGridObject.prototype._correctRowHeight=function(id,ind){
	...
	this.rowsAr[id].style.height=this._fake.rowsAr[id].style.height=Math.max(h,h2)+"px";
}
