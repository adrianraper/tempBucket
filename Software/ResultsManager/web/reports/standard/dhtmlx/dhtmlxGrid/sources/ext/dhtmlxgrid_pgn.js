...
dhtmlXGridObject.prototype._pgn_block=function(sep){ 
	...
	for (var i=start+1; i<=max; i++)
		str.push("<a href='#' onclick='this.grid.changePage("+i+"); return false;'>"+(i==this.currentPage?("<b>"+i+"</b>"):i)+"<a>");
	return str.join(sep);
}
dhtmlXGridObject.prototype._pgn_link=function(mode,ac,ds){
	if (mode=="prevpages" || mode=="prev"){
		if (this.currentPage==1) return ds;
		return '<a href=\'#\' onclick=\'this.grid.changePageRelative(-1*this.grid.pagesInGroup); return false;\'>'+ac+'</a>'
	}
	
	if (mode=="nextpages" || mode=="next"){
		if (this.rowsBuffer.length/this.rowsBufferOutSize <= this.currentPage ) return ds;
		if (this.rowsBuffer.length/(this.rowsBufferOutSize*this.pagesInGroup) <= 1 ) return ds;
		return '<a href=\'#\' onclick=\'this.grid.changePageRelative(this.grid.pagesInGroup); return false;\'>'+ac+'</a>'
	}
	
	if (mode=="current"){
		var i=this.currentPage+(ac?parseInt(ac):0);
		if (i<1 || Math.ceil(this.rowsBuffer.length/this.rowsBufferOutSize) < i ) return ds;
		return '<a href=\'#\' '+(i==this.currentPage?"class='dhx_active_page_link' ":"")+'onclick=\'this.grid.changePage('+i+'); return false;\'>'+i+'</a>'
	}
	return ac;
}
