...
dhtmlXGridObject.prototype.enableAutoHiddenColumnsSaving = function(name,cookie_param){
		this.setColumnHiddenSSC=this.setColumnHidden;
		this.setColumnHidden=function(){ this.setColumnHiddenSSC.apply(this,arguments); this.saveHiddenColumnsToCookie(name,cookie_param); }
}
...
dhtmlXGridObject.prototype.saveHiddenColumnsToCookie=function(name,cookie_param){
    ...
    if (this._fake)
        for (var i=0; i < this._fake._cCount; i++) {
            this._hrrar=this._fake._hrrar?this._fake._hrrar[i]:false;
        }
	this.setCookie(name,cookie_param,4,(this._hrrar||[]).join(",").replace(/display:none;/g,"1"));
...
dhtmlXGridObject.prototype.loadHiddenColumnsFromCookie=function(name){
	...
	z=(z||"").split(",");
	if (z.length>this._cCount) return;
	for (var i=0; i < z.length; i++) 
		this.setColumnHidden(i,(z[i]?true:false));
}
