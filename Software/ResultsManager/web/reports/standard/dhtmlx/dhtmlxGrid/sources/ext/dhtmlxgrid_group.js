...
dhtmlXGridObject.prototype._inc_group=function(val,hidden){
	if (val==="") val=" ";
	if (!this._groups[val]){ 
...
dhtmlXGridObject.prototype._addPseudoRow=function(skip){
	...
	r.appendChild(t);
	t.className="group_row";
	t.colSpan=this._cCount;
	
	var that=this;
	t.onclick=function(e){ if (that._fake && that._realfake) 
			that._fake._switchGroupState(that._fake._groups[that.getFirstParentOfType(this,"TR")._cntr.text].row); 
		else
			that._switchGroupState(that.getFirstParentOfType(this,"TR")); 
...
dhtmlXGridObject.prototype._switchGroupState=function(row){
	if (this._fake && !this._realfake) this._fake._switchGroupState(this._fake._groups[row._cntr.text].row)
	var z=row._cntr;
	...
	this._updateGroupView(z);
	this.callEvent("onGroupStateChanged",[z.row.idd]);
	this.setSizes();
}
/**
