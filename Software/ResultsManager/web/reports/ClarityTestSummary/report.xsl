					...
					<table id="reportTable" style="width:100%" gridWidth="100%">
						...
						<xsl:for-each select="report/row">	
							<tr>
								...
								<td>
									<xsl:choose>
										<xsl:when test="@grammarCorrect + @vocabularyCorrect + @listeningCorrect + @selfAssessment &gt; 100"> Advanced </xsl:when>
										<xsl:when test="@grammarCorrect + @vocabularyCorrect + @listeningCorrect + @selfAssessment &gt; 80"> Upper Intermediate </xsl:when>
										<xsl:when test="@grammarCorrect + @vocabularyCorrect + @listeningCorrect + @selfAssessment &gt; 60"> Intermediate </xsl:when>
										<xsl:when test="@grammarCorrect + @vocabularyCorrect + @listeningCorrect + @selfAssessment &gt; 40"> Pre-Intermediate </xsl:when>
										<xsl:when test="@grammarCorrect + @vocabularyCorrect + @listeningCorrect + @selfAssessment &gt; 20"> Elementary </xsl:when>
										<xsl:otherwise> Beginner </xsl:otherwise>
									</xsl:choose>
								</td>
