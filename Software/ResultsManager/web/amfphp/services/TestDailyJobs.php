<?php
/*
 * This script mimics the daily cron job - but is for testing individual sections
 */
...
$testingSection = 'listCompletedTests';
if (isset($_REQUEST['action'])) $testingSection = $_REQUEST['action'];
...
function runDailyJobs($triggerDate = null) {
	...
	echo "Running $testingSection for $expiryDate $newLine$newLine";
	
	// 1. Archive expired users for Road to IELTS Last Minute
	if ($testingSection == 'archiveExpiredRoadToIELTS') {
	...
	if ($testingSection == 'archiveExpiredAccounts') {
	...
	if ($testingSection == 'archiveOldUsers') {
	...
	if ($testingSection == 'EmailMeCBuilder') {
	...
	if ($testingSection == 'archiveSentEmails') {
	...
	if ($testingSection == 'countCBuilderActivity') {
	...
	if ($testingSection == 'listCompletedTests') {
        // a. Get list of test completions ordered by account
        ...
        foreach ($roots as $root) {
            ...
            echo "DPT results check account " . $account->name . "$newLine";
            ...
            if ($requireSummaryEmail) {
                ...
                foreach ($thisService->dailyJobOps->collateTestEmails($root, $completedTests) as $completedTest) {
                    echo $completedTest['name'] . ' completed a test and got ' . $completedTest['result'] . "$newLine";
                }
            }
            echo "$newLine";
        }
