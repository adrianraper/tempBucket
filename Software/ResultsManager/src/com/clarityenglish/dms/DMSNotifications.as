package com.clarityenglish.dms {
	
	/**
	 * ...
	 * @author ...
	 */
	public class DMSNotifications {
		public static const STARTUP:String = "startup";
		public static const ACCOUNTS_LOADED:String = "accounts_loaded";
		public static const ACCOUNT_DETAILS_LOADED:String = "account_details_loaded";
		public static const ADD_ACCOUNT:String = "add_account";
		public static const UPDATE_ACCOUNTS:String = "update_accounts";
		public static const DELETE_ACCOUNTS:String = "delete_accounts";
		public static const EMAIL_TEMPLATES_LOADED:String = "email_templates_loaded";
		public static const REPORT_TEMPLATES_LOADED:String = "report_templates_loaded";
		public static const CHANGE_EMAIL_TO_LIST:String = "change_email_to_list";
		public static const EMAIL_TO_LIST_CHANGED:String = "email_to_list_changed";
		public static const SHOW_IN_RESULTS_MANAGER:String = "show_in_results_manager";
		public static const ACCOUNTS_RESET:String = "accounts_reset";
	}
	
}