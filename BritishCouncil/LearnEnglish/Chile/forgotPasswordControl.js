...
$(function() {
	...
	// Form validation 
	$("input#forgotPasswordSubmit").click(function() {
		seemsOK = true;
		...
		if (studentID == "" && email == "") {
			seemsOK = false;
			$.blockUI({ message: $('#passwordForgetNoDetails') });
	...
	sendMethodToAction = function() {
		...
		$("input#forgotPasswordSubmit").hide();
		
		var studentID = $("input#loginID").val();
		var email = $("input#userEmail").val();
		var formData = "method=forgotPassword&loginID=" + studentID + "&email=" + email;
	...
	onAjaxSuccess = function(data, textStatus) {
		...
		$("input#forgotPasswordSubmit").show();
		$("div#responseMessage").text("");
		$.unblockUI();
