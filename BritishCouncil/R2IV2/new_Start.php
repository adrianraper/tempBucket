<?php	
	...
	if (isset($_SESSION['rootID'])) {
		...
		throw new Exception('no rootID passed');
	}
	if (isset($_SESSION['studentID']) && $_SESSION['studentID']!='') {
		...
		throw new Exception('no studentID passed');
	}
	...
	$startControl = "http://www.roadtoielts.com/Software/ResultsManager/web/";
	...
	// BC HK
	//throw new Exception('rootID=' + $rootID + " studentID=" + $studentID);
	if ($rootID==14028) {
		$prefix = 'TEST';
	...
	//if (isset($_SESSION['studentID'])) $studentID = rawurlencode($_SESSION['studentID']); 
