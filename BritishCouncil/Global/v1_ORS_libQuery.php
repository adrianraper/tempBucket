<?php
	...
	$domain = "http://dock.fixbench/BritishCouncil";
	...
	*/ 
	...
	function sendAndLoad($postXML, &$contents) {
		...
		global $commonDomain;
		global $startFolder;
		global $debugLog, $debugLogFile, $errorLog, $errorLogFile;
		/**
		...
		*/
		$targetURL = $commonDomain."Software/Common/Source/plain SQLServer/runProgressQuery.php";
		...
		*/
		if($contents === false){
			if ($errorLog) {
				error_log(curl_error($ch), 3, $errorLogFile);
			}
			curl_close($ch);
		} else {
			if ($debugLog) {
				error_log($contents, 3, $debugLogFile);
			}
			curl_close ($ch);
		}		
	}
