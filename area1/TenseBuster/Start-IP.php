<?php
	...
	$referrer = $ip = $server = $productCode = $courseFile = '';
	...
	//if (file_exists(dirname(__FILE__).'/'.$licenceFile)) {
	//	echo dirname(__FILE__).'/'.$licenceFile.' exists'; 
	//} else {
	//	echo dirname(__FILE__).'/'.$licenceFile.' is not there'; 
	//}
	//exit();
	...
	if (isset($_SESSION['TenseBuster'])) {
		$version = $_SESSION['TenseBuster']->languageCode;
	...
	// This is done in location-xx.txt - or has to be done in db
	/*
	switch ($version) {
		case "ZH":
		case "ZHO":
...
<head>
	...
	<script type="text/javascript">
		...
		// v6.5.6 For preview from RM - overwrite anyother conflicting parameters
