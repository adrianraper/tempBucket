<?php
	...
	# IYJ login.php uses PREFIX so it should be included as well
	} elseif (isset($_SESSION['PREFIX'])) {
		$prefix = $_SESSION['PREFIX'];
	} else {
	...
	//if (isset($_SESSION['ItsYourJob'])) {
	//	$version = $_SESSION['ItsYourJob']->languageCode;
	if (isset($_SESSION['LANGUAGECODE'])) {
		$version = $_SESSION['LANGUAGECODE'];
...
<head>
	<title>
		...
		switch ($version) {
		case "NAMEN":
			echo "Clarity English | It's Your Job | Practice Center";
			break;
		case "INDEN":
			echo "Clarity English | It's Your Job | Practice Center";
			break;
		default:
			echo "Clarity English | It's Your Job | Practice Centre";
		}
		?>
	</title>
