<?php
	...
	// if the argList is passed with encryption, we need to decrypt here
	if (isset($_GET['encrypt'])) {
		$ekey='clarity';
		$pattern = '/ /';
		$replacement = '+';
		$encrypt = preg_replace($pattern, $replacement, $_GET['encrypt']);
		$tempArgList = decrypt3DES(buildLoginHash($ekey), base64_decode(decodeCharacters($encrypt)));
	}
	
	// Passing the whole argList is fine, but at least we have to extract out the prefix
	$argListArray = doubleExplode ('&','=',$tempArgList);
	...
	} elseif (isset($argListArray['prefix'])) {
		$prefix = $argListArray['prefix'];
	} else {
	...
	*/
	...
	if (isset($_SESSION['UserID'])) $userID = $_SESSION['UserID'];
		elseif (isset($argListArray['userID'])) $userID = $argListArray['userID'];
	if (isset($_SESSION['UserName'])) $userName = $_SESSION['UserName'];
		elseif (isset($argListArray['userName'])) $userName = $argListArray['userName'];
	if (isset($_SESSION['Password'])) $password = $_SESSION['Password'];
		elseif (isset($argListArray['password'])) $password = $argListArray['password'];
	if (isset($_SESSION['StudentID'])) $studentID = $_SESSION['StudentID'];
		elseif (isset($argListArray['studentID'])) $studentID = $argListArray['studentID'];
	if (isset($_SESSION['Email'])) $Email = $_SESSION['Email'];
		elseif (isset($argListArray['email'])) $Email = $argListArray['email'];
	if (isset($_SESSION['InstanceID'])) $instanceID = $_SESSION['InstanceID'];
		elseif (isset($argListArray['instanceID'])) $instanceID = $argListArray['instanceID'];
...
<head>
	...
	<script type="text/javascript">
		...
		argList+="&productCode=<?php echo $productCode ?>";
		<?php if (isset($tempArgList)) { ?> //if there are anything that we haven't extracted out then will pass in here
			argList+="&<?php echo $tempArgList?>";
		<?php } ?>
		//var argList="?<?php echo $_GET['encrypt'];?>";
